...
const showInteractionCursor = () => {
    ...
    native.disableControlAction(0, 63, true);
    native.disableControlAction(0, 64, true);
    ...
    if (Date.now() > lastRayCast) {
        ...
        const [_, hit, endCoords, surfaceNormal, entity] = screenToWorld(raycastFlags, -1);
        if (!hit) {
            rayCastInfo = null;
            return;
        }
    ...
    if (native.isDisabledControlJustReleased(0, 24)) {
        ...
        alt.emitServer("player:SetPosition", new Vector3(rayCastInfo.endCoords.x, rayCastInfo.endCoords.y, rayCastInfo.endCoords.z + 2));
        rayCastInfo = null;
    }
};
...
const addVector3 = (vector1, vector2) => {
    return {
...
const subVector3 = (vector1, vector2) => {
    return {
        x: vector1.x - vector2.x,
        y: vector1.y - vector2.y,
        z: vector1.z - vector2.z,
...
export const processCoordinates = (x, y) => {
...
export const s2w = (camPos, relX, relY) => {
...
export const screenToWorld = (flags, ignore) => {
    ...
    return native.getShapeTestResult(ray, false, new alt.Vector3(0, 0, 0), new alt.Vector3(0, 0, 0), 0);
};
