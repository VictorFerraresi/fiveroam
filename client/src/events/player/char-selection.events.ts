...
let robberSelectionCam: number | undefined;
let copPed: number | undefined;
let robberPed: number | undefined;
let selectedTeam = 0;
...
export const onCharSelection = async () => {
  ...
  robberPed = native.createPed(
    4,
    alt.hash("g_m_y_ballasout_01"),
    -473.037353515625,
    -1039.3450927734375,
    52.4652099609375,
    130,
...
alt.on("keydown", (key) => {
  if (37 == key) {
    // Left arrow
    if (!isSelectingTeam || selectedTeam == 0) return;
    ...
    // Right arrow
    if (!isSelectingTeam || selectedTeam == 1) return;
    native.playSoundFrontend(
      1,
      "SELECT",
      "HUD_FRONTEND_DEFAULT_SOUNDSET",
...
const showHints = () => {
  ...
  drawText(
    `Pressione ~y~ENTER ~w~para selecionar`,
    0.5,
    0.9,
    0.6,
    1,
  ...
  native.addTextComponentSubstringPlayerName(
    `Use ~INPUT_FRONTEND_LEFT~ ~INPUT_FRONTEND_RIGHT~ ~INPUT_FRONTEND_UP~ ~INPUT_FRONTEND_DOWN~`
  );
